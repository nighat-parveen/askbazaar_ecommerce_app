...
export class OffersComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    const sliderContainer =
      this.ele.nativeElement.querySelector('.banner-slider');
    const sliderImages =
      this.ele.nativeElement.querySelectorAll('.banner-slider img');
    ...
    sliderContainer.style.transform =
      'translateX(' + -size * currentIndex + 'px)';
    ...
    prev.addEventListener('click', () => {
      ...
      currentIndex--;
      sliderContainer.style.transform =
        'translateX(' + -(size * currentIndex * 4) + 'px)';
    });
